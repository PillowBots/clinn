...
const webSearchTool = {
  name: "web_search",
  description: "搜索互联网获取实时信息. 用于查文档/新闻/最新资料",
  parameters: {
  ...
  execute: async ({ query, num }) => {
    ...
    try {
      const searchUrl = `https://www.google.com/search?q=${encodeURIComponent(query)}&num=${n}`;
      ...
      const snippets = (result.match(/<div class="BNeawe s3v9rd AP7Wnd">([^<]+)<\/div>/g) || [])
        .slice(0, n).map((s, i) => {
          const text = s.replace(/<[^>]+>/g, "").trim();
          return `${i + 1}. ${text}`;
        });
      return snippets.length > 0 ? `[搜索: ${query}]\n${snippets.join("\n")}`
        : `[搜索: ${query}] 未获取到有效结果(可能需要使用 web_fetch 访问具体URL)`;
    } catch (e) {
      return `[搜索失败] ${e.message}. 建议用 web_fetch 直接访问相关页面`;
    }
  },
};
